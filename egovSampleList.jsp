...
<body style="text-align:center; margin:0 auto; display:inline; padding-top:100px;">
    <form id="listForm" name="listForm" method="post" action="<c:url value="/egovSampleList.do"/>">
        ...
        <div id="content_pop">
        		...
        		<ul>
        			<li>
        				...
        				<select id="searchCondition" name="searchCondition" class="use">
        					<option value="1" <c:if test='${params.searchCondition eq "1"}'>selected="selected"</c:if> >Name</option>
        					<option value="0" <c:if test='${params.searchCondition eq "0"}'>selected="selected"</c:if> >ID</option>
        				</select>
        				
        			</li>
        			<li><label for="searchKeyword" style="visibility:hidden;display:none;"><spring:message code="search.keyword" /></label>
                        <input class="txt" name="searchKeyword" id="searchKeyword" value="${params.searchKeyword}"></input>
                    </li>
        			<li>
        	...
        	<div id="table">
        		<table width="100%" border="0" cellpadding="0" cellspacing="0" summary="카테고리ID, 케테고리명, 사용여부, Description, 등록자 표시하는 테이블">
        			...
        			<c:forEach var="result" items="${resultList}" varStatus="status">
            			<tr>
            				<td align="center" class="listtd"><c:out value="${paginationInfo.totalRecordCount - ((params.pageIndex-1) * paginationInfo.recordCountPerPage + status.index)}"/></td>
        	...
        	<div id="paging">
        		...
        		<input type="hidden" name="pageIndex" id="pageIndex" value="${params.pageIndex}"></input>
        	</div>
        	<div id="sysbtn">
        	  <ul>
        	      <li>
